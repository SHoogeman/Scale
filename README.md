...
1. [scale.set_offset()](scale_arduino_v2/scale_arduino_v2.ino#L52) is used to set the zero weight off set. 
2. [scale.set_scale()](scale_arduino_v2/scale_arduino_v2.ino#L53) is used to set the scales gradient. 
...
![image](Images/Webb_app_link.png) Copy the web-app url and replace the [link](ESP32/ESP32.ino#L10)

To connect the ESP32 to wifi change the [ssid and password](ESP32/ESP32.ino#L12-13) 

## start measurement 
To begin a measurement the sd should be inserted into the SD module. If the SD card is not inserted an error will be shown via the Serial monitor. To set the time interval between measurements adjust [measure_interval](scale_arduino_v2/scale_arduino_v2.ino#L31), the default is set to 1 minute. To set the starting time adjust [RTC.setHours and RTC.setMinutes](scale_arduino_v2/scale_arduino_v2.ino#L58-59). Now plug the arduino into an outlet via the USB B port and everthing should run smoothly. 
